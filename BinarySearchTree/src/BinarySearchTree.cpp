...
// if this node is null then return (avoid crashing)
	...
	if (bidId.compare(node->bids.bidId) < 0) {
		...
		//two children
			...
			node->bids = temp->bids;
			node->right = removeNode(node->right, temp->bids.bidId);
		}
...
void BinarySearchTree::addNode(Node* node, Bid bid) {
	...
	// if node greater than bid, add to left subtree
